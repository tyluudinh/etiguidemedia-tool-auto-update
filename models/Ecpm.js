...
let Ecpm = {
  ...
  service: function (app, startDate,  endDate, cookie, ratio, callback) {
    RequestData.getData(this.url_request(app.app_valuepotion_id, startDate, endDate), cookie, function (res) {
      ...
      var model = Ecpm.newModel(app.id, date);
      ...
      Ecpm.findOne(app.id, date, function (err, rows) {
        if (rows.length === 0){
          Ecpm.insert(model, function (err, rows) {
            callback(err, rows);
          })
        }
      })
  ...
  url_request: (appId, startDate, endDate) => {
    return `${utils.getEnv('URL_CHART') + appId}/summary_ecpm.chart?start_date=${startDate}&end_date=${endDate}&type=standard&collection_id=${appId}`
  }
};
